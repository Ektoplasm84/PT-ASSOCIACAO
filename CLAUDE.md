...
- **Vault upload `section` must be read from `req.body.section` only inside the multer handler, never in middleware that runs before `vaultUpload.single(...)`** — `req.body` is empty until multer finishes parsing the multipart stream, so a pre-multer middleware reading `req.body.section` always sees `undefined`. `vaultStorage.diskStorage.destination()` reads `req.body.section` correctly because it fires mid-parse (after the `section` field, since the vault upload forms always place the hidden `section` input before the `vault_file` input) — the handler after `vaultUpload.single(...)` must derive `section` the same way, from the now-fully-parsed `req.body.section`, or the DB row's `section` column desyncs from the folder the file was actually written to.
- **NIA fetch-photo type check must be duplicated on both the captcha route and the fetch-photo route** — `GET /members/:id/arc-captcha` and `POST /members/:id/arc-fetch-photo` each independently check `is_aprc`/`is_tw_passport`/`is_tw_id` and refuse. `arcSessions` tokens are keyed only by a random UUID (not bound to a member id), so a valid token obtained via one member's captcha step could otherwise be replayed against a blocked member's `:id` on the fetch route alone.
- **Member delete order: DB delete → file cleanup → audit write** (`routes/admin.js` `/members/:id/delete`) — never write the audit log before the destructive `DELETE FROM users` has actually run; a DB-level failure (FK/lock) would otherwise leave a false "deleted" audit entry pointing at a member row that still exists.
- **Card upload replace (delete old + insert new document row) runs inside a single `db.transaction()`** (`routes/admin.js` `/members/:id/documents/card`) — prevents an orphaned file-less DB gap if the `INSERT` fails after the old row's `DELETE` already committed.
- **Server log viewer is `super_admin` only on both routes** — `GET /admin/logs` (page) and `GET /admin/logs/stream` (SSE feed, live server console output) must use the identical guard (`role !== 'super_admin'`); the stream route previously also allowed plain `admin`, silently exposing runtime log content — do not let the two routes' guards drift apart again.
...
- **jimp v1.x API** — use `const { Jimp } = require('jimp')` (named export, NOT default); `Jimp.read(path)` to load images; `img.resize({ w: 300 })` (NOT `.resize(300, Jimp.AUTO)`); `img.write(dest)` to save. `Jimp.fromFile()` does NOT exist on the destructured export — always use `Jimp.read()`. Do not revert to v0.x patterns (`.writeAsync()`, `Jimp.AUTO`).
...
- **Calendar events** — created by admin/SA/gestao via dashboard modal; `resolveAudience()` in `routes/admin.js` converts audience group strings to user IDs via a single UNION query; invites inserted in the same transaction as the event; `end_date` must be ≥ `start_date` (validated server-side); `GET /admin/events/:id/invites` uses `calendarWriteGuard` (same guard as event create/delete) so any management user who can create an event can also see its RSVP list — not `adminOnly`, which would incorrectly block gestao/board/etc. from viewing invites on events they created themselves
...
- **Export Data fields picker** — `admin/export.ejs` lets admins toggle which XLSX columns to include via grouped checkboxes (Identity & Contact / Membership & Fees / Residence Document / Cartão de Cidadão / Personal & Education / Other) plus Select All/None; selection is posted as `fields[]` in the `POST /admin/export` body. `member_id` is always force-included regardless of the `fields` array sent — enforced in `utils/export.js`'s `_runExport` (`new Set(['member_id', ...selectedFields...])`), not in the `routes/admin.js` route handler — never trust the client to keep the row identifier. `utils/export.js` exports `FIELD_DEFS` (the master `{header,key,width}` column list); `startExportJob`/`_runExport` filter `ws.columns` to the selected keys but still build the full row-data object per member — ExcelJS ignores row keys that have no matching column, so the row-building code does not need to branch on selection. **Every key in `FIELD_DEFS` must have a matching key in the row object built in `_runExport`** — `city_zh`, `district_zh`, `district_en`, and `nationality` were previously defined as exportable columns but never populated in the row data, so those columns were silently blank in every export regardless of what the admin selected; when adding a new exportable field, add it to both `FIELD_DEFS` and the row-data object in the same change.
...
`city_zh` and `district_zh` are separate OCR-populated fields (editable in `member-form.ejs`/`profile-edit.ejs`), resolved against `taiwan-districts.js`. `district_en` is OCR-only — no form input — set via `apply-card-fields`, preserved on save via `district_en || member.district_en || null`, and displayed appended in parentheses next to `district_zh` on `member-detail.ejs` and `profile.ejs` (e.g. `南區 (South District)`). `utils/ocr.js`'s `postProcess()` prefers AI-extracted `city_zh`/`district_zh` over the `parseAddressCity()` regex fallback — the fallback only fires when the AI didn't return `city_zh`/`district_zh` directly; it does not overwrite AI values that are already present.
...
| `.pta-docslots` | Auto-fit responsive grid (`repeat(auto-fit,minmax(140px,1fr))`) for the seven card document slots |
